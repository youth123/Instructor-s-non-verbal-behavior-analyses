...
from resnet_cbam import cbam_resnet50
...
model_dir = args.backbone + "MixUp_model"
...
for alpha in range(1, 6):
    alpha = alpha / 10
    mixup_dir = os.path.join(model_dir, str(alpha))
    if not os.path.exists(mixup_dir):
        os.mkdir(mixup_dir)
    print("mixup_dir: ", mixup_dir)
    total_best_acc = []
    ...
    for exp_i in range(5):
        ...
        for epoch in range(60):
            ...
            for data in trainLoader:
                ...
                index = torch.randperm(img_feature.size(0)).cuda()
                inputs = lam * img_feature + (1 - lam) * img_feature[index, :]
        ...
        # print(state)
        save_path = os.path.join(mixup_dir, "{}_{}_{}_classroomClassification.pkl".format(str(best_acc), args.backbone, str(exp_i)))
        torch.save(state, save_path)

        import csv
        import os
        fileName = args.backbone + ".csv"
        filePath = os.path.join(mixup_dir, fileName)
    ...
    with open(filePath, "a+", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["average", str(average_best_acc), str(average_train_epoch_time), str(average_test_epoch_timie)])
